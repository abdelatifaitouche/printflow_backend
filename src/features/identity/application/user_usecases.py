...
from src.features.identity.application.dtos.user_dto import CreateUser, LoginUser

from src.features.identity.application.dtos.tokens import Tokens
...
class UserUC:
    ...
    async def register_user(self, data: CreateUser) -> None:
        async with self.uow:
            ...
            hashed_password: str = self.password_hasher.hash_password(data.password)
    ...
    async def login(self, creds: LoginUser):
        async with self.uow:
            user: User | None = await self.user_repo.get_by_email(email=creds.email)
